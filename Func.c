//
// Created by Tsimophey on 31-Mar-22.
//

#include "Func.h"

void rand_str(char *str, int len) {
    int i;
    for (i= 0; i < len; ++i) {
        str[i]=rand()%26+97;
    }
    str[i]='\0';
}

struct series* generate(struct series* c, int size) {
    c=(struct series*)malloc(sizeof(struct series) * size);
    printf("Enter data in format:\nName\tEpisode number\tRelease date \tSeason number\n (0 to random)\n");
    getc(stdin);
    for (int i = 0; i < size; ++i) {
        //fgets(c[i].Name, 30 * sizeof(char), stdin);
        gets(c[i].Name);
        if(c[i].Name[0]=='0'){
            rand_str(c[i].Name,10);
            c[i].epNumber=rand()%30+1;
            c[i].validator=rand()%2;
            if(c[i].validator){
                c[i].data.sNumber=rand()%10+1;
            } else{
                rand_str(c[i].data.data,10);
            }

        }
        else{
            scanf_s("%d",&c[i].epNumber);
            printf("data(0) or sNumer(1)?\n");
            int tmp=0;
            scanf_s("%d",&tmp);
            if(!tmp){
                getc(stdin);
                gets(c[i].data.data);
                c[i].validator=0;
            }
            else {
                c[i].validator=1;
                scanf_s("%d", &c[i].data.sNumber);
            }
        }
        printf("next\n");
    }
    return c;
}

void print_str(struct series *c) {
    if(c==NULL)return;
    printf("Name:\t\t%s\n",c->Name);
    printf("Episode number:\t%d\n",c->epNumber);
    if(c->validator!=0)
        printf("Season number:\t%d\n",c->data.sNumber);
    else
    printf("Release date:\t%s\n",c->data.data);

}

void print_br_str(struct series *c) {
    printf("Name:\t\t%s\n",c->Name);
    printf("Episode number:\t%d\n",c->epNumber);
}



void print_list(struct series *c, int num) {
    if(c==NULL){
        printf("\tERROR\n No data");
        return;
    }
    for (int i = 0; i < num; ++i) {
        print_str(&c[i]);
        printf("\n");
    }
}

struct series *find_by_data_data(struct series *c, int size, const char* parameter) {
    if((parameter==NULL)||(c==NULL)){
        printf("\tERROR\n Incorrect input\n");
        return NULL;
    }
    for (int i = 0; i < size; ++i) {
        int j=0,flag=0;
        while (c[i].data.data[j]!='\0'){
            if((c[i].data.data[j]!=parameter[j])){
                flag=1;
                break;
            }
            j++;
        }
        if(flag==0){
            return (c+i);
        }
    }
    return NULL;
}
void delete_element(struct series *list, int *size, struct series *element) {
    if((element==NULL)||(list==NULL)){
        return;
    }
    if (*size==1){
        *size=0;
        return;
    }
    do{
        *element= *(element+1);
        ++element;
    }
    while (element!=&(list[*size-1]));
    *size=*size-1;
}
struct series *find_min(struct series *list, int size){
    int min=INT_MAX;
    struct series *ptr=NULL;
    for (int i = 0; i < size; ++i) {
        if((list[i].validator==1)&&(list[i].data.sNumber<min)){
            min=list[i].data.sNumber;
            ptr=&list[i];
        }
    }
    return ptr;
}